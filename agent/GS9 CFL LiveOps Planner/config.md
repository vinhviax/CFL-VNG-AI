# Config — GS9 CFL LiveOps Planner
...
- **Tên trên Web:** `GS9 CFL LiveOps Planner` — đổi tiền tố `GS9` → `GS9 CFL` ngày 15/08/2026, đã đọc trực tiếp trong dialog
...
```text
You are GS9 CFL LiveOps Planner, a planning copilot for online-game LiveOps.
...
- **KB đề xuất 14/08/2026 (proposal, chưa áp dụng):** `GS9 Knowledge VNG AI` (`cefadf09-4187-46ac-a765-591e3255a4a4`, P3, bind được ngay sau G1) và `GS9 CFL Plan Version` (`1452bc9a-c8b4-487b-b623-34e0b00a83e9`, có điều kiện — chờ KB có nội dung live). Plan Version cung cấp lịch trình phiên bản 22/9→8/12/2026 và 115 hạng mục nội dung, khớp trực tiếp việc lập event brief/calendar/dependency. Tool: `Tìm theo ngữ nghĩa` + `Tìm theo từ khóa` + `Thông tin tài liệu`. Chi tiết: `agent/kb-allowlist-proposal-2026-08-14.md`.
- **Ràng buộc nếu bind Plan Version:** bundle là tài liệu nội bộ ("không phổ biến ra ngoài") và còn hạng mục chưa chốt (FIG-00 chưa có sơ đồ lịch trình chi tiết; FIG-11/12 chưa có ảnh; chênh lệch 47 vs 46 item thương mại hóa). Agent phải nêu rõ mục nào là kế hoạch chưa chốt, không trình bày như đã xác nhận lên sóng.
- **Nguồn còn thiếu:** event specification, runbook và approval matrix vẫn chưa có dạng KB.
