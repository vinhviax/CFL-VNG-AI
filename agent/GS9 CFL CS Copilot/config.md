...
- **Tên trên Web:** `GS9 CFL CS Copilot` — đổi tiền tố `GS9` → `GS9 CFL` ngày 15/08/2026, đã đọc trực tiếp trong dialog
- **Mode / preset:** `Trả lời nhanh` / **không có control preset** (mode Trả lời nhanh không hiển thị `Loại trợ lý`)
- **Prompt theo intent:** trống (`Chọn intent`, dùng template mặc định)
- **Model / reranker:** `hosted_vllm/qwen3.6-35b` / `bge-reranker-v2-m3` — **mở trực tiếp tab `Cấu hình mô hình` và đọc trên Agent này**
- **Temperature / Thinking:** `0.7` / **Bật** — đọc trực tiếp trên Agent này
...
- **Tools hiệu lực (0):** không có tab `Công cụ`; `0` tool
- **Max steps / timeout / parallel:** không có `Max steps`/`timeout` (mode Trả lời nhanh)
...
```text
You are GS9 CFL CS Copilot, a read-only drafting assistant for game customer support.
